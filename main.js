...
var SemanticGraphView = class extends import_obsidian.ItemView {
  ...
  render() {
    ...
    const buildResultsPanel = (bm25Hits, semHits) => {
      var _a2;
      resultsPanel.empty();
      const all = /* @__PURE__ */ new Map();
      for (const { id: id2, score } of bm25Hits) all.set(id2, { bm25: score, sem: 0 });
      for (const { id: id2, score } of semHits) {
        const ex = (_a2 = all.get(id2)) != null ? _a2 : { bm25: 0, sem: 0 };
      ...
      for (const [id2, { bm25, sem }] of sorted) {
    ...
    findBtn.addEventListener("click", async () => {
      ...
      semIndicator.textContent = total > 0 ? `BM25: ${bm25Ids.size}  ~: ${semIds.size}` : "no results";
      semIndicator.style.display = "inline";
