...
var SemanticGraphView = class extends import_obsidian.ItemView {
  ...
  render() {
    ...
    const applySearch = () => {
      ...
      (_c2 = this.selEdgeLine) == null ? void 0 : _c2.style("opacity", (d) => matchIds.has(d.source.id) || matchIds.has(d.target.id) ? "0.6" : "0.05");
    ...
    for (const t of presentTypes) {
      const color2 = (_b = NODE_COLORS[t]) != null ? _b : "#888";
      const shape = (_c = NODE_SHAPES[t]) != null ? _c : "circle";
    ...
    this.pendingRaf = requestAnimationFrame(() => {
      ...
      nodeEl.on("mouseenter", (_ev, d) => {
        ...
        tooltip.innerHTML = `<strong>${d.title}</strong><br><span class="llm-tp-type" style="color:${(_c2 = NODE_COLORS[d.type]) != null ? _c2 : "#888"}">${d.type}</span>` + (d.domain ? `<br><em>${d.domain}</em>` : "") + `<br><small>degree: ${deg} \xB7 neighbors: ${neighbors.size}</small><br><small class="llm-tp-hint">click to select \xB7 click again to open</small>`;
